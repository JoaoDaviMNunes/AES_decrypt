...
def salvar_texto(texto_decifrado, chave):
...
def incrementar_chave(chars, chave):
...
def decifrar_aes_ecb(dados_hex, chave):
    # Converte o texto hexadecimal para bytes
    bytes_encriptados = binascii.unhexlify(dados_hex)
    # Cria uma instância do AES com a chave e o modo ECB
    cifra = AES.new(chave.encode('utf-8'), AES.MODE_ECB)
    # Decifra os bytes
    bytes_decifrados = cifra.decrypt(bytes_encriptados)
    # Remove padding e converte os bytes decifrados para texto
    try:
        texto_decifrado = bytes_decifrados.decode('utf-8').strip()
    except UnicodeDecodeError:
        texto_decifrado = ""
    return texto_decifrado

# Função para verificar as chaves em paralelo
def verificar_chaves(dados_hex, chars, chave_inicial, chave_final, flag_end):
    chave = chave_inicial
    while chave is not None and chave != chave_final and not flag_end.is_set():
        print(f"Thread {threading.current_thread().name} verificando chave: {chave}")
        texto_decifrado = decifrar_aes_ecb(dados_hex, chave)
        salvar_texto(texto_decifrado, chave)
        chave = incrementar_chave(chars, chave)
...
# Chave inicial
chave_inicial = "SecurityAES00000"

# Definir número de threads/processos
num_threads = 8

# Calcular intervalos de chaves para cada thread/processo
intervalo_total = len(chars) ** 5
intervalo_por_thread = intervalo_total // num_threads

# Lista de threads
threads = []
flag_end = multiprocessing.Event()

# Criar e iniciar threads
for i in range(num_threads):
    chave_start = "SecurityAES" + ''.join([chars[0]] * 5)
    chave_end = "SecurityAES" + ''.join([chars[-1]] * 5)
    
    thread = threading.Thread(target=verificar_chaves, args=(dados_hex, chars, chave_start, chave_end, flag_end), name=f'Thread-{i}')
